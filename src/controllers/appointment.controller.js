...
const bookAppointment = asyncHandler(async (req, res) => {
  ...
  const lawyerUser = await User.findOne({ _id: lawyerId, role: "lawyer", isActive: true }).populate("lawyerProfile");
  ...
  if (existing) {
    throw new apiError(409, "Selected time slot is already booked for this lawyer");
  }
...
const getAvailableSlots = asyncHandler(async (req, res) => {
  ...
  if (!v) availableSlots = [];
    ...
    try {
      ...
      availableSlots = String(v).split(",").map(s => s.trim()).filter(Boolean);
    }
  }
